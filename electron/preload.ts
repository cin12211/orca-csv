...
const electronAPI = {
  ...
  csv: {
    ...
    validateFile: (filePath: string) =>
      ipcRenderer.invoke('csv:validate-file', filePath),
    readFile: (filePath: string) =>
      ipcRenderer.invoke('csv:read-file', filePath),
    writeFile: (filePath: string, content: string) =>
      ipcRenderer.invoke('csv:write-file', { filePath, content }),
    watchFile: (filePath: string) =>
      ipcRenderer.invoke('csv:watch-file', filePath),
    unwatchFile: (filePath: string) =>
      ipcRenderer.invoke('csv:unwatch-file', filePath),
    onFileChanged: (
      filePath: string,
      callback: (eventType: string) => void
    ) => {
      const handler = (_: IpcRendererEvent, eventType: string) =>
        callback(eventType);
      ipcRenderer.on(`csv:file-changed:${filePath}`, handler);
      return () =>
        ipcRenderer.removeListener(`csv:file-changed:${filePath}`, handler);
    },
  },
};
