  ...
  <div class="w-full select-none h-9 flex items-center justify-between">
    ...
    <div class="flex items-center gap-1">
      ...
      <Tooltip v-if="!isReadOnly">
        <TooltipTrigger as-child>
          <Button variant="outline" size="xxs" class="font-normal" @click="emit('addRow')">
      ...
      <Tooltip v-if="hasSelection">
        ...
        <TooltipContent><p>Delete {{ selectedRowsCount }} selected rows</p></TooltipContent>
      </Tooltip>

      <p v-if="selectedRowsCount" class="font-normal text-xs text-primary/60">Selected</p>
      <p v-if="selectedRowsCount" class="font-normal text-sm text-primary">{{ selectedRowsCount }}</p>
    </div>

    <!-- Center: file meta -->
    <div v-if="fileName" class="flex items-center gap-3 text-xs text-muted-foreground">
      <span class="font-medium text-foreground max-w-[160px] truncate" :title="fileName">{{ fileName }}</span>
      <span v-if="fileSize" class="text-[10px]">{{ formatBytes(fileSize) }}</span>
      <span class="text-[10px] font-mono text-foreground/70">{{ rowCount }} × {{ columnCount }}</span>
    </div>
    ...
    <div class="flex items-center gap-1">
      ...
      <Tooltip>
        <TooltipTrigger as-child>
          <Button variant="ghost" size="xxs" class="gap-1.5 font-normal" @click="localHasHeaders = !localHasHeaders">
            <Checkbox id="has-headers-cb" :checked="localHasHeaders" class="pointer-events-none size-3" />
            <Label for="has-headers-cb" class="text-xs font-normal cursor-pointer select-none pointer-events-none">
              Headers
            </Label>
          </Button>
        </TooltipTrigger>
      ...
      <DropdownMenu>
        <DropdownMenuTrigger as-child>
          <Button variant="ghost" size="xxs" class="font-normal gap-1">
            ...
            <span class="text-xs max-w-[64px] truncate">{{ currentDelimiterLabel }}</span>
          </Button>
        </DropdownMenuTrigger>
        <DropdownMenuPortal>
          <DropdownMenuContent align="end" class="min-w-40">
            <p class="px-2 py-1.5 text-xs font-medium text-muted-foreground">Delimiter</p>
            <DropdownMenuSeparator />
